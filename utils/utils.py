...
def load_checkpoint_with_missing_or_exsessive_keys(cfg, model):
    # params loaded from ckpt
    state_dict = torch.load(cfg.general.checkpoint)["state_dict"]
    ckpt_param_names = list(state_dict.keys())
    # params expected for the model
    correct_dict = dict(model.state_dict())

    # name: param for everything thats loaded into the model -> can use to set a different LR for these params
    loaded_params = []

    # find the params that are in the checkpoint but not in the model
    for key in state_dict.keys():
    ...
    # params expected for the model
    correct_dict = dict(model.state_dict())
    # go through expected params in the model
    for key in correct_dict.keys():
        if key not in state_dict:
            logger.warning(f"{key} not in loaded checkpoint")
            state_dict.update({key: correct_dict[key]})  # missing, keep param from model
        elif state_dict[key].shape != correct_dict[key].shape:
            logger.warning(
                f"incorrect shape {key}:{state_dict[key].shape} vs {correct_dict[key].shape}"
            )
            state_dict.update({key: correct_dict[key]}) # wrong shape, keep param from model

    # now state_dict has everything from the model and the checkpoint
    # the final params to load
    new_state_dict = dict()

    for key in state_dict.keys():
        if key in correct_dict.keys():
            new_state_dict.update({key: state_dict[key]}) # keep this param
            if key in ckpt_param_names: 
                loaded_params.append(key) # params that were loaded from the ckpt
        else:
            logger.warning(f"excessive key: {key}") # if we have more keys just discard them

    # should have all the keys required to load into model
    model.load_state_dict(new_state_dict)
    return cfg, model, loaded_params
