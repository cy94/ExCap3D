...
class ScannetPreprocessing(BasePreprocessing):
    def __init__(
            ...
            save_dir: str = "/cluster/eriador/cyeshwanth/caption3d/mask3d/scannet_data_2",
            # tsv_path="/canis/Datasets/ScanNet/public/v2/scannet-labels.combined.tsv",
            tsv_path="/cluster/eriador/cyeshwanth/caption3d/mask3d/scannetv2-labels.combined.tsv",
            modes: tuple = ("train", "validation"),
            n_jobs: int = -1,
            git_repo: str = "/rhome/cyeshwanth/old-repos/ScanNet",
        ...
        if self.scannet200:
            self.labels_pd = pd.read_csv(self.tsv_path, sep='\t', header=0)
        ...
        for mode in self.modes:
            ...
            # only 1 sample
            print(f'{mode}: Keep only 1 scene')
            filepaths = filepaths[:1]
            # sort the mesh paths - why?
    ...
    def process_file(self, filepath, mode):
        ...
        npy format: coords (3), RGB 0-255 (3), normals (3), unique seg ID (1), semantic labels(0-40 from PLY) (1), instance ID (1)
        gt txt format: semantic label * 1000 + instance label + 1
        """
