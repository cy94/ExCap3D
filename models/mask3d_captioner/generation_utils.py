...
def greedy_decode(transformer: Callable, **kwargs) -> Tensor:
    ...
    for word_id in range(max_length):
        ...
        temporal_inputs = torch.cat([
            inputs_embeds, 
            transformer.transformer.wte(output_ids[:, :word_id+1])
        ], dim=1)
...
def beam_search_decode(transformer: Callable, **kwargs) -> Tensor:
    ...
    output_ids = output_ids * kwargs['eos_token_id']
    # have cross attn values -> store attns
    if kwargs['encoder_hidden_states'] is not None:
        num_ctx = kwargs['encoder_hidden_states'].shape[1]
        # originally: list of (nobj/batch_size, num_heads, caption sequence_length, context sequence_length) for each layer
        # here we'll keep only the last layer cross atn
        # 4 = num heads
        output_cross_attns = torch.zeros(batch, nbeams, 4, max_length, num_ctx).to(inputs_embeds.device)
    else:
        output_cross_attns = None
    ...
    for word_id in range(max_length):
        ...
        if word_id == 0:    # cold start for the first generation step
            ...
            temporal_inputs = torch.cat([
                expanded_inputs_embeds, 
                transformer.transformer.wte(generated_words.long())
            ], dim=2)
            ...
            step_output = transformer(
                ...
                encoder_hidden_states=None if kwargs['encoder_hidden_states'] is None else (
...
def generation(transformer: Callable, **kwargs):
    ...
    kwargs['encoder_hidden_states'] = kwargs.get('encoder_hidden_states', None)
    ...
    if inputs_embeds is not None:
        ...
        kwargs['inputs_embeds'] = transformer.transformer.wte(input_ids)
    else:
