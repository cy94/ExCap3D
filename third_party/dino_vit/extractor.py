...
class ViTExtractor:
    ...
    def __init__(self, model_type: str = 'dino_vits8', stride: int = 4, model: nn.Module = None, device: str = 'cuda'):
        ...
        self.model = ViTExtractor.patch_vit_resolution(self.model, stride=stride) # change the stride, pos enc function
        self.model.eval()
        self.model.to(self.device)
        self.p = self.model.patch_embed.patch_size # patch size
        if type(self.p) != int:
            self.p = self.p[0] # should be a single int 
        self.stride = self.model.patch_embed.proj.stride # input stride?
    ...
    def patch_vit_resolution(model: nn.Module, stride: int) -> nn.Module:
        ...
        # fix the stride
        model.patch_embed.proj.stride = stride # originally (14,14 for dinov2)
    ...
    def preprocess_image(self, image: np.ndarray,
        ...
        if load_size is not None:
            prep_image = transforms.Resize(load_size, interpolation=transforms.InterpolationMode.LANCZOS)(image)
        else:
            prep_image = image
        ...
        transformed_image = transform(prep_image)[None, ...]
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        with Timer():
            ...
            print(f"Image {args.image_path} is preprocessed to tensor of size {image_batch.shape}.") # 1, 3, h, w
    ...
    torch.save(descriptors, args.output_path) # 1,1,1457=number of patches (sliding window), 384=fixed feature dim
    print(f"Descriptors saved to: {args.output_path}")

    num_patches = extractor.num_patches
    print(f'Feature dims: {num_patches[0]}, {num_patches[1]}') # feature h,w
    print(f'Patch size: {extractor.p}') # patch size
    print(f'Stride: {extractor.stride}') # stride
    
    features = descriptors.squeeze().cpu().numpy() # h*w, dim

    pca = PCA(n_components=3)
    pca.fit(features)

    pca_features = pca.transform(features) # h*w, 3
    pca_features = (pca_features - pca_features.min()) / (pca_features.max() - pca_features.min())
    pca_features = pca_features * 255 # h*w, 3
    print('PCA Features:', pca_features.shape)

    out_img_shape = num_patches
    print('Feat image shape:', out_img_shape)
    out_img = pca_features.reshape(num_patches[0], num_patches[1], 3).astype(np.uint8)
    # save small rgb 
    out_path = Path('dinov2_features_extractor_small.png')
    print('Saving feat viz to:', out_path)
    # RGB to BGR
    cv2.imwrite(str(out_path), cv2.cvtColor(out_img, cv2.COLOR_RGB2BGR))

    # load the original image to get its size 
    orig_img = np.array(o3d.io.read_image(args.image_path)) # h,w,3
    # reshape to original size and save again
    out_img = cv2.resize(out_img, orig_img.shape[:2][::-1])
    print('Original Viz image:', out_img.shape)
    out_path = Path('dinov2_features_extractor.png')
